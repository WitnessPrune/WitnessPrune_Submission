...
class FilterStatsCollector:  
    ...
    # def compute_classwise_filter_means(self, num_samples_per_class):
    #     num_classes = 10  # CIFAR10 has 10 classes
    #     filter_means = {f"Class_{i}": [] for i in range(num_classes)}
    #     filter_means.update({f"Complement_Class_{i}": [] for i in range(num_classes)})

    #     for class_label in range(num_classes):
    #         # DataLoader for the specific class
    #         class_loader = self.class_dataloader(class_label, num_samples=num_samples_per_class)
    #         mean_z_class = self.get_mean_z(class_loader, num_samples_per_class)
    #         filter_means[f"Class_{class_label}"] = mean_z_class

    #         # DataLoader for the complement of the class
    #         complement_loader = self.complement_dataloader(class_label, num_samples=num_samples_per_class)
    #         mean_z_complement = self.get_mean_z(complement_loader, num_samples_per_class)
    #         filter_means[f"Complement_Class_{class_label}"] = mean_z_complement

    #     return filter_means
    # def compute_classwise_filter_means(self, num_samples_per_class):
    #     num_classes = 10  # Assuming CIFAR10 has 10 classes
    #     filter_means = {f"Class_{i}": None for i in range(num_classes)}
    #     filter_means.update({f"Complement_Class_{i}": None for i in range(num_classes)})
    ...
    #         # For each class
    #         E_zj = class_means[f"Class_{i}"]
    #         E_Zj = class_means_Z[f"Class_{i}"]
    #         Sj_class[f"Class_{i}"] = E_Zj - torch.ger(E_zj.squeeze(), E_zj.squeeze())

    #         # For each class complement
    #         E_zj_complement = complement_means[f"Complement_Class_{i}"]
    #         E_Zj_complement = complement_means_Z[f"Complement_Class_{i}"]
    #         Sj_complement[f"Complement_Class_{i}"] = E_Zj_complement - torch.ger(E_zj_complement.squeeze(), E_zj_complement.squeeze())
    ...
    #     for i in range(num_classes):
    #         # For each class
    #         E_zj = class_means[f"Class_{i}"].squeeze()
            
    #         E_Zj = class_means_Z[f"Class_{i}"]

    #         if E_zj.ndim > 1:
    #             E_zj = E_zj.view(-1)  # Flatten the tensor to 1D

    #         # Debugging prints
    #         print(f"E_zj shape: {E_zj.shape}")
    #         print(f"E_Zj shape: {E_Zj.shape}")
    #         print(f"torch.ger(E_zj, E_zj) shape: {torch.ger(E_zj, E_zj).shape}")

    #         Sj_class[f"Class_{i}"] = E_Zj - torch.ger(E_zj, E_zj)

    #         # For each class complement
    #         E_zj_complement = complement_means[f"Complement_Class_{i}"].squeeze()
    #         E_Zj_complement = complement_means_Z[f"Complement_Class_{i}"]
    #         if E_zj_complement.ndim > 1:
    #             E_zj_complement = E_zj_complement.view(-1)  # Flatten the tensor to 1D
    #         Sj_complement[f"Complement_Class_{i}"] = E_Zj_complement - torch.ger(E_zj_complement, E_zj_complement)
    ...
    #         # S_c = self.Sj_class[class_label]
    #         # S_cp = self.Sj_complement[class_label]
    ...
    #         # S = S_c + S_cp
    #         # print(S.shape)
    #         # Initialize an array to hold Fisher scores for each filter
    #         T_scores = torch.zeros(num_filters)

    #         for j in range(num_filters):
    #             delta_m = m_c[j] - m_cp[j]
    #             S = S_c[j] + S_cp[j]
    #             print(S.shape, delta_m.shape)
    #             # Calculate Fisher Discriminant for each 
    #             # print(delta_m[j].shape, S[j].shape)
    #             # F_c = torch.dot(delta_m[j].T, torch.linalg.inv(S[j]) @ delta_m[j])
    #             F_c = torch.dot(delta_m.T, torch.linalg.inv(S) @ delta_m)
    #             T_c = F_c / (2 + F_c)
    #             print(j, F_c, T_c, saliency_scores[j])
    #             T_scores[j] = T_c
